...
async function updateArticle() {
  ...
  const { error } = await supabase.from('posts').insert({
    ...
    category: 'スポーツ・アウトドア',
    thumbnail_url: 'https://ggmcgokdtmflioqezrqk.supabase.co/storage/v1/object/public/shop-thumbnails/fairway-golf.webp'
  });

  if (error) console.error('Error inserting article:', error);
  else console.log('Article updated successfully via delete-then-insert strategy.');
}
